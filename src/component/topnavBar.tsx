...
const NavBar = () => {
  ...
  const navigation: NavigationItem[] = [
    { name: "Brands", href: "#", current: true, icon: true },
    { name: "Shop", href: "#", current: false, icon: true },
    { name: "Premium", href: "#", current: false },
    { name: "Gift Cards", href: "#", current: false },
  ];

  return (
    <nav className="bg-shark h-20 mx-auto  w-full">
      <div className="flex items-center justify-between h-16 px-4">
        <div className=" ">
          <ul className="text-left sm:hidden xl:flex">
            {navigation.map((item, index) => (
              <li className="pl-0 inline-flex items-center pr-4">
                <Link href={item.href} key={`Link${index}`} className="text-white lowercase mt-2 font-bold hover:text-corn">{item.name}
                  {item.icon && <ArrowDropDownIcon className="w-4 h-4" />}</Link>
                  <span className=" hover:border-b-corn hover:transform hover:origin-left hover:scale-x-0 hover:transition-transform hover:duration-300 hover:ease-in-out group-hover:scale-x-100 group-hover:transition-transform group-hover:duration-300"></span>
              </li>
            ))}
          </ul>

          {!isMobileMenuOpen && <MenuIcon sx={{ color: "#fff" }} onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="xl:hidden" />}
          {isMobileMenuOpen && <CloseIcon sx={{ color: "#fff" }} onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} />}
        </div>
        <div className="flex-shrink-0">
          <Image
            className="w-full h-full"
      ...
      {isMobileMenuOpen && <div className="w-full  bg-shark visible min-h-full">
        <MenuList
